...
def fetch_all_expense_data(slug: str, year: int, consuntivo_only: bool=False):
    if year <= 2015:
        base = f"http://storico.openbilanci.it/bilanci/{slug}/spese/dettaglio"
    else:
        base = f"https://openbilanci.it/armonizzati/bilanci/{slug}/spese/dettaglio"
...
for _, row in df.iterrows():
    ...
    expense_data = fetch_all_expense_data(slug, prev_year)
    ...
    if expense_data:
        row_data.update(expense_data)
        all_columns.update(expense_data.keys())
    else:
...
with open("expense_2015_backwards.csv", "w", newline='', encoding='utf-8') as f:
...
print("✅ Saved to expense_2015_backwards.csv")
